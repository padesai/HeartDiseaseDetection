...
for j in range (0, len(images_all)):
    ...
    saver = tf.train.import_meta_graph('../heartdiseasedetectionmodel_systole_latest.meta')
    ...
    ventricle_volumes.append([dirs[j] + "_Systole", result[0][0]])

    g += 1

csv.register_dialect('volumeDialect', delimiter=',', quoting=csv.QUOTE_NONE, escapechar="\\")

volumeFile = open('volume_systole.csv', 'w', newline='')
with volumeFile:
